`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// 파일 : tb_fault_manager_axi_A11_irq_gating.v
...
module tb_fault_manager_axi_A11_irq_gating;
    ...
    initial begin
        ...
        axi_read (ADDR_IRQ_EN, rd);
        check32(rd, 32'h00000000, "IRQ_EN disabled");

        timeout = 3'b001;
        repeat (6) @(posedge ACLK);

        axi_read(ADDR_IRQ_STATUS, rd);
        check32(rd, 32'h00000001, "pending set while IRQ_EN=0");
        check_bit(irq, 1'b0, "irq stays low while IRQ_EN=0");

        axi_write(ADDR_IRQ_EN, 32'h00000001);
        repeat (3) @(posedge ACLK);
        check_bit(irq, 1'b1, "irq rises after IRQ_EN=1");

        axi_write(ADDR_IRQ_STATUS, 32'h00000001);
        repeat (3) @(posedge ACLK);
        axi_read(ADDR_IRQ_STATUS, rd);
        check32(rd, 32'h00000000, "pending cleared by W1C");
        check_bit(irq, 1'b0, "irq low after pending clear");
