`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// 파일 : tb_fault_manager_axi_A12_count_packing.v
...
module tb_fault_manager_axi_A12_count_packing;
    ...
    initial begin
        ...
        $display("=== A12 : FAULT_COUNT count1/count2 packing 검증 ===");

        axi_write(ADDR_IRQ_EN, 32'h00000000);
        axi_write(ADDR_IRQ_STATUS, 32'h00000001);

        // Device 1 Count Packing: bit[15:8]
        timeout        = 3'b000;
        error_flag     = 3'b010;
        ...
        checks = checks + 1;
        if (rd[7:0] !== 8'd0 || rd[15:8] !== 8'd4 ||
            rd[23:16] !== 8'd0 || rd[31:24] !== 8'd0) begin
            errors = errors + 1;
            $display("  [FAIL] count1 packing got=0x%08h exp=0x00000400", rd);
        end
        else $display("  [ ok ] count1 packing  (0x%08h)", rd);

        check32({24'd0, dut.fault_count1}, 32'd4, "internal count1 == 4");

        // Count 초기화: Disable 후 Enable
        error_flag = 3'b000;
        repeat (4) @(posedge ACLK);
        axi_write(ADDR_CTRL, 32'h00000000);
        repeat (4) @(posedge ACLK);
        axi_write(ADDR_CTRL, 32'h00000001);
        repeat (4) @(posedge ACLK);

        // Device 2 Count Packing: bit[23:16]
        // Device 2가 Critical로 즉시 처리되지 않도록 mask=000
        axi_write(ADDR_CRITICAL_MASK, 32'h00000000);
        timeout        = 3'b000;
        error_flag     = 3'b100;
        critical_fault = 3'b000;
        wait_ticks(3);
        ...
        axi_read(ADDR_FAULT_COUNT, rd);
        checks = checks + 1;
        if (rd[7:0] !== 8'd0 || rd[15:8] !== 8'd0 ||
            rd[23:16] !== 8'd4 || rd[31:24] !== 8'd0) begin
            errors = errors + 1;
            $display("  [FAIL] count2 packing got=0x%08h exp=0x00040000", rd);
        end
        else $display("  [ ok ] count2 packing  (0x%08h)", rd);

        check32({24'd0, dut.fault_count2}, 32'd4, "internal count2 == 4");
